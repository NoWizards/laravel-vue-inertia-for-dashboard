    ...
    <section class="grid grid-cols-1 lg:grid-cols-2 gap-2">
        <Box v-for="listing in listings.data" :key="listing.id" :class="{ 'bg-amber-100': listing.deleted_at }">
            ...
            <div class="flex items-center gap-1 text-gray-600 dark:text-gray-300">
                <a
                class="btn-outline text-xs font-medium bg-white" 
                :href="route('listings.show', { listing: listing.id })"
                target="_blank"
                >Preview</a>
                <Link class="btn-outline text-xs font-medium bg-white" :href="route('realtor.listing.edit', { listing: listing.id })">Edit</Link>
                <Link
